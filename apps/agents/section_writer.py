...
class SectionWriterAgent(BaseAgent):
    ...
    def _user_prompt(self, state: PipelineState, task: SectionWriteTask) -> str:
        ...
        return (
            ...
            f"Target length: approximately {task.target_words} words; do not write "
            f"less than {max(60, int(task.target_words * 0.85))} words unless the "
            "brief is impossible to expand without filler.\n"
