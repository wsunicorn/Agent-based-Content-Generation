...
- [ ] Lưu `source_map`.
- [ ] Ghi `AgentRun` thật cho từng agent/node.
- [ ] Ghi thời gian bắt đầu/kết thúc từng agent.
- [ ] Ghi prompt snapshot ngắn gọn, tránh lưu quá dài.
- [ ] Ghi response snapshot ngắn gọn.
- [ ] Ghi số call, input token, output token nếu provider trả về.
- [ ] Ghi `Revision` thật khi QA/router yêu cầu sửa.
- [ ] Hiển thị timeline agent trên dashboard.
- [ ] Hiển thị agent nào chạy lại trong revision.
...
- [ ] Tạo template riêng cho `blog_post`.
- [ ] Tạo template riêng cho `technical_report`.
- [ ] Tạo template riêng cho `news_article`.
- [ ] Tạo template riêng cho `tutorial`.
- [ ] Blog post cần có:
  - [ ] Hook
  - [ ] Reader problem
  - [ ] Practical sections
...
- [ ] Technical report cần có:
  ...
  - [ ] Recommendations
- [ ] News article cần có:
  - [ ] Lead theo who/what/when/where/why
  - [ ] Context
...
- [ ] Tutorial cần có:
  ...
  - [ ] Troubleshooting
  - [ ] Next steps
- [ ] Outline Agent phải chọn template theo `content_type`.
- [ ] Writer phải tuân thủ template.
- [ ] Editor kiểm tra format theo template.
- [ ] QA có điểm `format_adherence`.

---

## Phase 6 - Domain-Specific Assistant

- [ ] Thêm field `domain` cho Job.
- [ ] Các domain ban đầu:
  - [ ] Tech
  - [ ] Marketing
  - [ ] Education
...
- [ ] Tạo `domain_guides`.
- [ ] Tạo `style_guides`.
- [ ] Tạo `forbidden_claims` hoặc cảnh báo theo domain nhạy cảm.
- [ ] Thêm domain-specific source preference.
- [ ] Thêm domain terminology/glossary.
- [ ] Prompt của Research/Writer/Editor/QA nhận domain guide.
- [ ] UI đổi từ generic "AI Content Pipeline" sang domain-focused assistant.
- [ ] Cho user chọn domain ở form tạo job.
- [ ] Cho user chọn audience/tone rõ ràng.

---

## Phase 7 - Research và Fact-check mạnh hơn

- [ ] Cache Tavily/search result theo topic và keywords.
- [ ] Cache nội dung scrape theo URL.
- [ ] Lưu source document thành artifact riêng.
- [ ] Research output nên có structured fields:
  ...
  - [ ] `source_credibility`
  - [ ] `source_dates`
- [ ] FactChecker map claim về source cụ thể.
- [ ] Claim không có source phải bị flag.
...
- [ ] Thêm chế độ không web search khi không có Tavily key.
- [ ] Nếu research yếu, router quay lại Research với query mới.

---

## Phase 8 - Hỗ trợ dữ liệu ngoài văn bản

- [ ] Thêm model hoặc artifact type cho media.
- [ ] Hỗ trợ upload PDF.
- [ ] Hỗ trợ upload image.
- [ ] Hỗ trợ upload audio/video hoặc URL video.
- [ ] PDF ingestion bằng `pypdf`, `pymupdf`, hoặc `unstructured`.
- [ ] Image OCR bằng Tesseract/EasyOCR hoặc vision model nếu có.
- [ ] Audio/video transcript bằng Whisper local.
- [ ] Media Research Agent chuyển media thành text evidence.
- [ ] Lưu transcript/OCR output thành artifact.
- [ ] Writer có thể dùng media evidence.
- [ ] SEO Agent sinh image alt text/caption nếu có media.
- [ ] Export bài kèm citation/caption từ media.
...
- [ ] Thêm mode `fast`.
- [ ] Thêm mode `standard`.
- [ ] Thêm mode `high_quality`.
- [ ] Fast mode bỏ bớt FactChecker hoặc QA chuyên sâu.
- [ ] Standard mode dùng local writer/editor và Gemini structured tasks.
...
- [ ] Hiển thị agent timeline rõ hơn.
- [ ] Hiển thị sources/evidence panel.
- [ ] Hiển thị outline trước khi viết.
- [ ] Cho user approve/edit outline trước khi chạy writer.
- [ ] Cho user regenerate từng section.
...
- [ ] Cho phép export:
  ...
  - [ ] DOCX
  - [ ] PDF
- [ ] Thêm trạng thái worker/Redis/DB trên health panel.
...
4. [ ] AgentRun/Revision logging thật.
5. [ ] Content templates và domain guide.
