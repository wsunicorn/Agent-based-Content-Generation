...
- [x] Không hardcode `DJANGO_SETTINGS_MODULE=config.settings.development` trong ASGI.
- [x] Không hardcode `DJANGO_SETTINGS_MODULE=config.settings.development` trong Celery.
- [x] Đọc settings từ environment khi deploy.
- [x] Thêm `CSRF_TRUSTED_ORIGINS`.
- [x] Thêm `SECURE_PROXY_SSL_HEADER`.
- [x] Cấu hình `ALLOWED_HOSTS` production rõ ràng.
- [x] Thêm Dockerfile production.
- [x] Thêm `docker-compose.prod.yml`.
- [x] Thêm startup command cho web:
  - [x] migrate
  - [x] collectstatic
  - [x] start Daphne
- [x] Thêm startup command cho worker:
...
- [x] Tách service:
  ...
  - [x] Redis
  - [x] Postgres
- [x] Thêm health check endpoint đầy đủ.
- [x] Thêm deployment notes cho Render.
- [x] Thêm deployment notes cho Railway.
- [x] Ghi rõ local LLM không phù hợp free hosting nếu không có máy riêng/VPS đủ mạnh.
- [x] Nếu deploy public, thêm auth hoặc simple password protection.
